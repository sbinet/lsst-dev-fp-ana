...
func NewFileScanner(name string) lsst.P {
	...
	proc.Config = func(opts lsst.Options) error {
		...
		ctx.fout, err = fits.Create(fname)
		if err != nil {
			return err
		}

		phdu, err := fits.NewPrimaryHDU(&ctx.fout, fits.NewDefaultHeader())
		if err != nil {
		...
		ctx.tbl, err = fits.NewTableFrom(&ctx.fout, "fpfsum", ForcedPhotData{}, fits.BINARY_TBL)
		if err != nil {
			return err
		}
		return err
	...
	proc.Proc = func(f lsst.File) error {
		var err error
